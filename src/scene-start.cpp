...
void drawMesh(SceneObject sceneObj)
{
    ...
    // Part B - Rotate the object correctly when holding the left and middle mouse
    // buttons while moving horizontally and vertically.

    // Take XYZ angles from sceneObj struct and multiply them in a rotation variable
    // translate model by multiplying with rotation and scale.
...
void display(void)
{
    ...
    // Part A - Rotating the camera correctly when holding the left and middle mouse
    // buttons.
    ...
    // Multiply matrices by current view
    view = view * xRotate * yRotate;
...
// Part C - Introducing ambient, diffuse, specular and shine functions.
// Introduce both functions and alter the menu functions required for their
// implementation.
...
static void materialMenu(int id)
{
    ...
    // Part C - Implement callback functions for the menu for
    // ambient, diffuse, specular and shine.
    else if (id == 20)
    {
        toolObj = currObject;
...
static void makeMenu()
{
    ...
    // Part C - Remove "unimplemented" id = 20
...
void reshape(int width, int height)
{
    ...
    // Part D - Allow the camera to get closer without clippng through the
    // object.
    GLfloat nearDist = 0.02;

    // Part E - Modify the viewing window so that when it gets resized, whatever
    // is displayed within the window follows suit.
    GLfloat left;
    GLfloat right;
    GLfloat bot;
    GLfloat top;
